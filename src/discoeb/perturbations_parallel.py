...
def evolve_modes( *, tau_max, tau_out, param, kmodes: Sequence, 
                        lmaxg : int, lmaxgp : int, lmaxr : int, lmaxnu : int,
                        nqmax : int, rtol: float, atol: float,
                        pcoeff : float, icoeff : float, dcoeff : float, factormax : float, factormin : float, max_steps : int  ) -> Sequence:
    ...
    def model_synchronous_parallel(tau, y, param: Sequence):
        return jnp.concatenate([model_synchronous( tau=tau, y=_y, param=_param[0], kmode=_param[1],  
                                                   lmaxg=lmaxg, lmaxgp=lmaxgp, lmaxr=lmaxr, lmaxnu=lmaxnu, nqmax=nqmax) 
                                                   for  _y, _param in zip( y.reshape(len(kmodes, nvar)), param)])

    modelX_ = VectorField( model_synchronous_parallel)
    modelX = drx.ODETerm( modelX_ )
    


    # ... determine starting time
    tau_start = jnp.concatenate([determine_starting_time( param=param, k=_kmode ) for _kmode in kmodes])
    tau_start = 0.99 * jnp.array([jnp.minimum( jnp.min(tau_out), _tau_start ) for _tau_start in zip( tau_start)])

    # ... set adiabatic ICs
    y0 = jnp.concatenate([adiabatic_ics_one_mode( tau=_tau_start, param=param, kmode=_kmode, nvar=nvar, 
                       lmaxg=lmaxg, lmaxgp=lmaxgp, lmaxr=lmaxr, lmaxnu=lmaxnu, nqmax=nqmax ) for _tau_start, _kmode in
                       zip(tau_start, kmodes)])
    
    filter_indices = jnp.concatenate([jnp.array([0,2,3,5,6,7]) + nvar*index for index, _ in kmodes])
    weights = jnp.concatenate([ jnp.array([1,kmode**2,1,1,1/kmode**2,1]) for kmode in kmodes])

    # create solver wrapper, we use the Kvaerno5 solver, which is a 5th order implicit solver
    def DEsolve_implicit( *, model, t0, t1, y0, saveat, norm_params: List[jnp.ndarray] ):
        return drx.diffeqsolve(
            ...
            y0=y0,
            saveat=saveat,  
            stepsize_controller = drx.PIDController(
                rtol=rtol, 
                atol=atol, 
                norm=lambda t:rms_norm_filtered(t, *norm_params), 
            ...
            max_steps=max_steps,
            args=jnp.stack((param, kmodes, )), # do i need to repeat param?
    ...
    sol = DEsolve_implicit( 
        model=modelX, 
        t0=jnp.max(tau_start), 
        t1=jnp.min(tau_max), # don´t know how to fix yet, should be different 
        y0=y0, 
        saveat=saveat,
        norm_params = [filter_indices, weights]  
        # norm_params=[jnp.array([0,2,3,5,6,7]), jnp.array([1,kmode**2,1,1,1/kmode**2,1]) ] 
        )

    # convert outputs
    yout = jax.vmap( lambda y : [convert_to_output_variables( y=_y, param=param, kmode=_kmode, 
                                                   lmaxg=lmaxg, lmaxgp=lmaxgp, lmaxr=lmaxr, lmaxnu=lmaxnu, nqmax=nqmax) for _y, _kmode in zip(y.reshape(len(kmodes), nvar), kmodes)] )( sol.ys )
...
def evolve_perturbations( *, param, aexp_out, kmin : float, kmax : float, num_k : int,
    ...
    #                                 in_axes=0
    # )(kmodes)
    y1 = evolve_modes(tau_max=tau_max, 
                      tau_out=tau_out,
                      param=param,
                      kmodes=kmodes, # Sequence
                      lmaxg=lmaxg, 
                      lmaxgp=lmaxgp,
                      lmaxr=lmaxr, 
                      lmaxnu=lmaxnu, 
