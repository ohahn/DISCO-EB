...
def model_synchronous(*, tau, yin, grhom, grhog, grhor, param, kmode, lmaxg, lmaxgp, lmaxr, lmaxnu, nqmax):
    ...
    photbar = grhog / (grhom * param['Omegab'] * a)
    ...
    grho = (
        grhom * param['Omegam'] / a
        + (grhog + grhor * (param['Neff'] + param['Nmnu'] * rhonu)) / a**2
        + grhom * param['OmegaL'] * a**2
        + grhom * param['Omegak']
    )

    gpres = (
        (grhog + grhor * param['Neff']) / 3.0 + grhor * param['Nmnu'] * pnu
    ) / a**2 - grhom * param['OmegaL'] * a**2
    ...
    dgrho = (
        grhom * (param['Omegac'] * deltac + param['Omegab'] * deltab) / a
        + (grhog * deltag + grhor * (param['Neff'] * deltar + param['Nmnu'] * drhonu)) / a**2
    )
    dgpres = (
        grhog * deltag + grhor * param['Neff'] * deltar
    ) / a**2 / 3.0 + grhor * param['Nmnu'] * dpnu / a**2
    ...
    dgtheta = (
        grhom * (param['Omegac'] * thetac + param['Omegab'] * thetab) / a
        + 4.0 / 3.0 * (grhog * thetag + param['Neff'] * grhor * thetar) / a**2
        + param['Nmnu'] * grhor * kmode * fnu / a**2
    )
...
@partial(jax.jit, static_argnames=('lmaxg', 'lmaxgp', 'lmaxr', 'lmaxnu', 'nqmax'))
def model_synchronous_neutrino_cfa(*, tau, yin, grhom, grhog, grhor, param, kmode, lmaxg, lmaxgp, lmaxr, lmaxnu, nqmax):
    """Solve the synchronous gauge perturbation equations for a single mode.
    ...
    dgrho = (
        grhom * (param['Omegac'] * deltac + param['Omegab'] * deltab) / a
        + (grhog * deltag + grhor * (param['Neff'] * deltar + param['Nmnu'] * rhonu * deltanu)) / a**2
    )
    dgpres = (
        grhog * deltag + grhor * param['Neff'] * deltar
    ) / a**2 / 3.0 + grhor * param['Nmnu'] * dpnu / a**2
    ...
    dgtheta = (
        grhom * (param['Omegac'] * thetac + param['Omegab'] * thetab) / a
        + 4.0 / 3.0 * (grhog * thetag + param['Neff'] * grhor * thetar) / a**2
        + param['Nmnu'] * grhor * rho_plus_p_nu * thetanu / a**2
    )
    etaprime = 0.5 * dgtheta / kmode**2
    alpha  = (hprime + 6.*etaprime)/2./kmode**2
    f = f.at[2].set( etaprime )
    ...
    dgshear = (
        4.0 / 3.0 * (grhog * shearg + param['Neff'] * grhor * shearr) / a**2
        + param['Nmnu'] * grhor * rho_plus_p_nu * shearnu / a**2
    )
...
def adiabatic_ics( *, tau: float, param, kmodes, num_k, nvar, lmaxg, lmaxgp, lmaxr, lmaxnu, nqmax):
    ...
    fracnu = grhor * (param['Neff'] + param['Nmnu']) * 4.0 / 3.0 / a2 / s

    # ... use yrad=rho_matter/rho_rad to correct initial conditions for matter+radiation
    yrad = (
        grhom * param['Omegam'] * a
        / (grhog + grhor * (param['Neff'] + param['Nmnu'] * rhonu))
    )
...
def evolve_one_mode( *, y0, tau_start, tau_max, tau_out, grhom, grhog, grhor, param, kmode, lmaxg, lmaxgp, lmaxr, lmaxnu, nqmax, rtol, atol ):

    model1 = diffrax.ODETerm( 
        lambda tau, y , params : 
            model_synchronous( tau=tau, yin=y, grhom=grhom, grhog=grhog, grhor=grhor, param=param, kmode=kmode, lmaxg=lmaxg, lmaxgp=lmaxgp, lmaxr=lmaxr, lmaxnu=lmaxnu, nqmax=nqmax ) 
    )
    ...
    model2 = diffrax.ODETerm( 
        lambda tau, y , params : 
            ...
            model_synchronous_neutrino_cfa( tau=tau, yin=y, grhom=grhom, grhog=grhog, grhor=grhor, param=param, kmode=kmode, lmaxg=lmaxg, lmaxgp=lmaxgp, lmaxr=lmaxr, lmaxnu=lmaxnu, nqmax=nqmax ) 
    )
...
def evolve_perturbations( *, param, aexp_out, kmin : float, kmax : float, num_k : int, \
    ...
    y1 = jax.vmap(
        lambda k_y0 : evolve_one_mode( y0=k_y0[1:], tau_start=tau_start, tau_max=tau_max, tau_out=tau_out, 
                                        grhom=grhom, grhog=grhog, grhor=grhor, 
