...
from pylinger_cosmo import nu_background, nu_perturb
from functools import partial
from jaxopt import Bisection
import diffrax

@partial(jax.jit, static_argnames=('lmaxg', 'lmaxgp', 'lmaxr', 'lmaxnu', 'nqmax'))
def model_synchronous(*, tau, yin, rhonu, pnu, omegam, omegab, param, kmode, lmaxg, lmaxgp, lmaxr, lmaxnu, nqmax):
    ...
    # photbar = param['grhog'] / (param['grhom'] * omegab * a)
    ...
    grho = (
        param['grhom'] * omegam / a
        + (param['grhog'] + param['grhor'] * (param['Neff'] + param['Nmnu'] * rhonu.evaluate(a))) / a**2
        + param['grhom'] * param['OmegaL'] * a**2
        + param['grhom'] * param['Omegak']
    )

    gpres = (
        (param['grhog'] + param['grhor'] * param['Neff']) / 3.0 + param['grhor'] * param['Nmnu'] * pnu.evaluate(a)
    ...
    dgrho = (
        param['grhom'] * (omegac * deltac + omegab * deltab) / a
    ...
    dgtheta = (
        param['grhom'] * (omegac * thetac + omegab * thetab) / a
...
# @partial(jax.jit, static_argnames=('lmaxg', 'lmaxgp', 'lmaxr', 'lmaxnu', 'nqmax'))
def model_synchronous_neutrino_cfa(*, omegam, omegab, tau, yin, param, kmode, lmaxg, lmaxgp, lmaxr, lmaxnu, nqmax):
    """Solve the synchronous gauge perturbation equations for a single mode.
...
def adiabatic_ics( *, tau: float, rhonu, pnu, omegam, omegab, param, kmodes, num_k, nvar, lmaxg, lmaxgp, lmaxr, lmaxnu, nqmax):
    ...
    yrad = (
        param['grhom'] * omegam * a
        / (param['grhog'] + param['grhor'] * (param['Neff'] + param['Nmnu'] * rhonu.evaluate(a)))
    )
...
def adiabatic_ics_one_mode( *, tau: float, rhonu, pnu, omegam, omegab, param, kmode, nvar, lmaxg, lmaxgp, lmaxr, lmaxnu, nqmax):
    """Initial conditions for adiabatic perturbations"""
    omegac = omegam - omegab
...
# @partial(jax.jit, static_argnames=('lmaxg', 'lmaxgp', 'lmaxr', 'lmaxnu', 'nqmax', 'nvar', 'rtol', 'atol'))
def evolve_one_mode( *, rhonu, pnu, tau_max, tau_out, omegam, omegab, param, kmode, lmaxg, lmaxgp, lmaxr, lmaxnu, nqmax, nvar, rtol, atol ):
    ...
    # model1_ = lambda tau, y , params : model_synchronous( omegam=params, omegab=omegab, rhonu=rhonu, pnu=pnu, tau=tau, yin=y, param=param, kmode=kmode, lmaxg=lmaxg, lmaxgp=lmaxgp, lmaxr=lmaxr, lmaxnu=lmaxnu, nqmax=nqmax ) 


    model1_ = VectorField( lambda tau, y , params : model_synchronous( omegam=params[0], omegab=omegab, rhonu=params[1], pnu=params[2], tau=tau, yin=y, param=param, kmode=kmode, lmaxg=lmaxg, lmaxgp=lmaxgp, lmaxr=lmaxr, lmaxnu=lmaxnu, nqmax=nqmax ) )
    model1 = diffrax.ODETerm( model1_ )
    ...
    #     lambda tau, y , params : 
    #         model_synchronous_neutrino_cfa( omegam=params, omegab=omegab, tau=tau, yin=y, param=param, kmode=kmode, lmaxg=lmaxg, lmaxgp=lmaxgp, lmaxr=lmaxr, lmaxnu=lmaxnu, nqmax=nqmax ) 
    # )
    ...
    y0 = adiabatic_ics_one_mode( omegam=omegam, omegab=omegab, rhonu=rhonu, pnu=pnu, tau=tau_start, param=param, kmode=kmode, nvar=nvar, 
    ...
    # return y0 + 0.1 * model1_( tau=tau_start, y=y0, params=omegam ) 
...
def evolve_perturbations( *, omegam : float, omegab : float, param, aexp_out, kmin : float, kmax : float, num_k : int, \
    ...
    nvar   = 5 #7 + (lmaxg + 1) + (lmaxgp + 1) + (lmaxr + 1) + nqmax * (lmaxnu + 1)
    ...
    num_thermo = 1000
    a = jnp.geomspace(aexp_out[0], aexp_out[-1], num_thermo)
    ...
    # pnu_spline =  jaxinterp.InterpolatedUnivariateSpline(a, pnu_)
    rhonu_coeff = diffrax.backward_hermite_coefficients(ts=a, ys=rhonu_)
    pnu_coeff = diffrax.backward_hermite_coefficients(ts=a, ys=pnu_)
    rhonu_spline = diffrax.CubicInterpolation( ts=a, coeffs=rhonu_coeff )
    pnu_spline = diffrax.CubicInterpolation( ts=a, coeffs=pnu_coeff )
    # set initial conditions
    # y0 = adiabatic_ics( omegam=omegam, omegab=omegab, rhonu=rhonu_spline, pnu=pnu_spline, tau=tau_start, param=param, kmodes=kmodes, num_k=num_k, nvar=nvar, 
    #                    lmaxg=lmaxg, lmaxgp=lmaxgp, lmaxr=lmaxr, lmaxnu=lmaxnu, nqmax=nqmax )
    
    # y1 = evolve_one_mode( omegam=omegam, omegab=omegab,  rhonu=rhonu_spline, pnu=pnu_spline, y0=y0[-1,:], tau_start=tau_start, tau_max=tau_max, tau_out=tau_out, 
    #                                     param=param, kmode=kmax, lmaxg=lmaxg, lmaxgp=lmaxgp, lmaxr=lmaxr, 
    #                                     lmaxnu=lmaxnu, nqmax=nqmax, rtol=rtol, atol=atol )
    # return y1
    # solve ODEs 
    y1 = jax.vmap( lambda kk : evolve_one_mode( omegam=omegam, omegab=omegab, rhonu=rhonu_spline, pnu=pnu_spline, tau_max=tau_max, tau_out=tau_out, 
                                        param=param, kmode=kk, lmaxg=lmaxg, lmaxgp=lmaxgp, lmaxr=lmaxr, 
                                        lmaxnu=lmaxnu, nqmax=nqmax, nvar=nvar, rtol=rtol, atol=atol ))(kmodes)
