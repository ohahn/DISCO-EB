...
def model_synchronous(*, tau, yin, param, kmode, lmaxg, lmaxgp, lmaxr, lmaxnu, nqmax, ):
    ...
    def calc_baryon_photon_uncoupled( f, w ):
        ...
        f = f.at[idxg+2].add( w * sheargprime )

        # photon temperature l>=3, BLT11 eq. (2.4d)
        ell  = jnp.arange(3, lmaxg )
        f = f.at[idxg+ell].add( w * (kmode  / (2 * ell + 1) * (ell * y[idxg+ell-1] - (ell + 1) * y[idxg+ell+1]) - opac * y[idxg+ell]) )
        # photon temperature hierarchy truncation, BLT11 eq. (2.5)
        f = f.at[idxg+lmaxg].add( w * (kmode * y[idxg+lmaxg-1] - (lmaxg + 1) / tau * y[idxg+lmaxg] - opac * y[idxg+lmaxg]) )
    
        # polarization equations, BLT11 eq. (2.4e)
        # photon polarization l=0
        f = f.at[idxgp+0].add( w * (-kmode * y[idxgp+1] - opac * y[idxgp] + 0.5 * opac * polter) )
        # photon polarization l=1
        f = f.at[idxgp+1].add( w * (kmode / 3.0 * (y[idxgp] - 2.0 * y[idxgp+2]) - opac * y[idxgp+1]) )
        # photon polarization l=2
        f = f.at[idxgp+2].add( w * (kmode * (0.4 * y[idxgp+1] - 0.6 * y[idxgp+3]) - opac * (y[idxgp+2] - 0.1 * s_l2 * polter)))
        # photon polarization lmax>l>=3
        ell  = jnp.arange(3, lmaxgp)
        f = f.at[idxgp+ell].add( w * (kmode  / (2 * ell + 1) * (ell * y[idxgp+ell-1] - (ell + 1) * y[idxgp+ell+1]) - opac * y[idxgp+ell]) )
        # photon polarization hierarchy truncation
        f = f.at[idxgp+lmaxgp].add( w * (kmode * y[idxgp+lmaxgp-1] - (lmaxgp + 1) / tau * y[idxgp+lmaxgp] - opac * y[idxgp+lmaxgp]) )
        
        return f
    
    def calc_baryon_photon_tca( f, w ):
    ...
    def tca_vs_uncoupled_weight():
        sharpness = 0.1
        c1 = tauc/tauk/tight_coupling_trigger_tau_c_over_tau_k-1.0
        c2 = tauc/tauh/tight_coupling_trigger_tau_c_over_tau_h-1.0
        c3 = tauc/tauk/(0.1*tight_coupling_trigger_tau_c_over_tau_k)-1.0
        w1 = 1.0/(1.0+jnp.exp(-c1/sharpness))
        w2 = 1.0/(1.0+jnp.exp(-c2/sharpness))
        w3 = 1.0/(1.0+jnp.exp(-c3/sharpness))
        alpha = jnp.maximum( w1, jnp.minimum(w2,w3) )
        return alpha
    
    alpha = tca_vs_uncoupled_weight() 
    
    f = calc_baryon_photon_uncoupled( f, alpha )
    f = calc_baryon_photon_tca( f, 1.-alpha )
...
def evolve_perturbations( *, param, aexp_out, kmin : float, kmax : float, num_k : int, \
                         lmaxg : int = 12, lmaxgp : int = 12, lmaxr : int = 12, lmaxnu : int = 12, \
